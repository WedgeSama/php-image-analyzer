...
class ColorClass {
	...
	static $colors = array(
			...
			"MANGENTA" => array(255, 0, 255),
			"CYAN" => array(0, 255, 255),
			"WHITE" => array(255, 255, 255),
			"BLACK" => array(0, 0, 0),
			//"VERTFONCE" => array(5, 157, 0),
	);
	 ...
	 * @return array
	 */
	private function organized($classification, $h, $w, $percent, $ratio) {
		$total = $h * $w;
		$result = array();

		foreach ($classification as $index => $nb) {
			if ($percent)
				$result[self::$palette_colors_link[$index]] = ($nb / $total) * 100;
			else
				$result[self::$palette_colors_link[$index]] = (int) ($nb * $ratio);
		}
	...
	public function getArrayDominance(Image $image, $percent = true) {
		...
		return $this
				->organized($classif, $image->getHeightT(), $image->getWidthT(), $percent, $image->getRatio());
	}
